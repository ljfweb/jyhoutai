<template>
  <div>
    <van-nav-bar fixed title="标题" left-arrow @click-left="$router.go(-1)">
      <template #title>
        <div>
          <van-search
    ...
    <div class="tips" v-if="!ishow" style="margin-top:44px;">
      <div class="zuijin">
        <span>最近搜索</span>
        <i @click="clearlist"></i>
      </div>
...
export default {
  ...
  methods: {
    getmylist(item) {
      this.getlist(1, item); //根据子组件传来的数据 从新请求
    ...
    getSearch() {
      ...
      this.s_list = JSON.parse(localStorage.list);
      this.ishow = true;
      this.getlist(1, this.searchText); //调取搜索
    }
  },
  data() {
    return {
