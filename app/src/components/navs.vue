  ...
  <div v-if="$store.state.flag"
    ...
    <van-tabbar v-model="active">
      <van-tabbar-item v-for="(item, index) in tabbars"
                       ...
                       :badge="item.name == 'cart' ? $store.state.allCartNum : ''"
                       :icon="$store.state.navsIndex== index ? item.active : item.normal">
        <!-- {{item.title}} -->
      </van-tabbar-item>
    </van-tabbar>
  </div>
...
export default {
  ...
  methods: {
    turn (name, index) {
      this.$store.state.navsIndex = index;
  ...
  data () {
    return {
      ...
      tabbars: [
        ...
        // {
        //   name: localStorage.token ? "mine" : "login",
        //   title: "我的",
        //   normal: localStorage.token
